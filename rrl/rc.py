...
class DESN_MaxInfo(Oger.nodes.SparseReservoirNode):
    """
    DESN-MaxInfo
    
    reservoirs is a list of formerly initiated reservoirs (e.g. SparseReservoirNode)
    
    .. todo::
        This implementation only works if the client reservoirs are SparseReservoirNodes
        Therefore, the reservoirs need not to be passed to the constructor but only the
        parameters, i.e. input_scaling, bias, spectral_radius, sizes, ... (as lists or
        scalars if identical for all reservoirs)
        
        It would also be nice to do the computation differently, such that any client
        reservoir can be used directly (maybe even using a mixin?). This way, also
        leaky neurons could be used, for example.
    ...
    with zx_i(n-1) = x_i(n)
     (unit-time forward operator)
    
    x(n) = f( W_in u(n) + W_MI x(n-1) + W_back d(n-1) )
    
    W_in = [ (W_1^in)^T, ..., (W_R^in)^T ]^T
    W_back = [ (W_1^back)^T, ..., (W_R^back)^T ]^T
    x(n) = [ x_1^T(n), ..., x_R^T(n) ]^T
    ...
    x_R = W_MI * x(n): (R*N)x(R*N) * (R*N)x(1) = (R*N)x(1)
    
    x_R = [
           W_1*x_1(n) + -1/S*I*x_2(n) + ... + -1/S*I*x_R(n)
        -z/S*I*x_1(n) +    W_2*x_2(n) + ... + -1/S*I*x_R(n)
        ...
        -z/S*I*x_1(n) + -z/S*I*x_2(n) + ... +    W_R*x_R(n)
    ]
    = [
           W_1*x_1(n) + -1/S*I*x_2(n) + ... + -1/S*I*x_R(n)
        -1/S*x_1(n+1) +    W_2*x_2(n) + ... + -1/S*I*x_R(n)
        ...
        -1/S*x_1(n+1) + -1/S*x_2(n+1) + ... +    W_R*x_R(n)
    ]
    
    with x_1(n+1) ~= W_1*x_1(n) + -1/S*I*x_2(n) + ... + -1/S*I*x_R(n) <- use this in x(n+1) = f( W_in u(n) + ... + W_back d(n-1) )
    
    W_1*x_1(n) - sum( x_k, k=2..R )
    """
    def __init__(self, reservoirs, nonlin_func=np.tanh, reset_states=True):
        self.reservoirs = reservoirs
        
        # parent constructor
        super(DESN_MaxInfo, self).__init__(input_dim=None, output_dim=None, spectral_radius=0.9,
             nonlin_func=nonlin_func, reset_states=reset_states, bias_scaling=0, input_scaling=1, dtype='float64', _instance=0,
    ...
    def initialize(self):
        # initialize client reservoirs
        range_ = [0]
        for r in self.reservoirs:
            if not r._is_initialized:
                r.initialize()
            range_.append(range_[-1] + r.output_dim)
        self.rranges = zip(range_[:-1], range_[1:])
        
        # Reservoir matrices
        self.w_in = scipy.sparse.vstack(map(lambda r: r.w_in, self.reservoirs),'csc')
        self.w_bias = np.concatenate(map(lambda r: r.w_bias, self.reservoirs),1)
        w_blocks = []
        for idx1 in range(len(self.reservoirs)):
            row = []
            for idx2 in range(len(self.reservoirs)):
                if idx1 == idx2:
                    row.append(self.reservoirs[idx1].w)
                else:
                    row.append(- scipy.sparse.eye(self.reservoirs[idx1].output_dim, self.reservoirs[idx2].output_dim) / (max(1.0, len(self.reservoirs) - 1.0)))
            w_blocks.append(row)
        self.w = scipy.sparse.bmat(w_blocks,'csc')
        
        # output and input dimension
        self.set_output_dim(sum(map(lambda r: r.output_dim, self.reservoirs)))
        self.set_input_dim(self.reservoirs[0].input_dim)
    ...
    def _execute(self, x, simulate=False):
        """
        x.shape = (steps, input_dimensions)
        y.shape = (steps, output_dimensions)
        reservoir._execute :: x -> y
        ...
        # Pre-compute W_in * u(n)
        # W_in: (R*N)x(M)
        # x: (steps)x(M)
        # ips: (R*N)x(steps)
        # Around double as fast as if computed within the loop
        ips = self.w_in * x.T
        
        # compute new state
        for n in range(steps):
            state = states[n,:].copy()
            for lo,hi in self.rranges:
                # state                     : (R*N) x 1
                # self.w_in * x[n,:]        : (R*N)xM * (steps)xM
                # ips[lo:hi,n]              : N x 1
                # self.w_bias[lo:hi]        : 1 x N
                # self.w[lo:hi,:] * state   : N x 1
                state[lo:hi] = nonlinear_function_pointer(ips[lo:hi,n] + self.w_bias[0,lo:hi] + self.w[lo:hi,:] * state) # ips[lo:hi,n].T !?
